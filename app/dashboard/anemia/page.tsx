...
import { Users, Activity, TrendingUp, AlertCircle, Plus, Eye, Loader2, ArrowLeft, Droplet } from "lucide-react"
...
export default function AnemiaDashboardPage() {
  ...
  const fetchDashboardData = async () => {
    try {
      setLoading(true)
      const response = await fetch("/api/dashboard/medtech")
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
                  ...
                  Anemia Screening Dashboard
                </h1>
                <p className="text-sm text-slate-600 dark:text-slate-400">Anemia Detection Analytics</p>
              </div>
            </div>
            <Link href="/screening/flow?type=MEDTECH">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          <Card className="border-slate-200 dark:border-slate-700 shadow-md hover:shadow-lg transition-shadow bg-white dark:bg-slate-800">
            ...
            <CardContent>
              <div className="text-3xl font-bold text-slate-900 dark:text-slate-100">{stats?.highRisk || 0}</div>
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-2">
                {stats?.completed ? Math.round((stats.highRisk / stats.completed) * 100) : 0}% of completed
              </p>
            </CardContent>
          </Card>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          ...
          <Card className="border-slate-200 dark:border-slate-700 shadow-md bg-white dark:bg-slate-800">
            <CardHeader>
              ...
              <CardDescription className="text-slate-600 dark:text-slate-400">Anemia risk levels</CardDescription>
            </CardHeader>
            <CardContent>
        ...
        <Card className="border-slate-200 dark:border-slate-700 shadow-md bg-white dark:bg-slate-800">
          <CardHeader>
            ...
            <CardDescription className="text-slate-600 dark:text-slate-400">Latest Anemia screening records</CardDescription>
          </CardHeader>
          <CardContent>
            {recentPatients.length > 0 ? (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-slate-200 dark:border-slate-700">
                      ...
                      <th className="text-left py-3 px-4 font-semibold text-slate-700 dark:text-slate-300">Responses</th>
                  ...
                  <tbody>
                    {recentPatients.map((patient) => (
                      <tr key={patient.id} className="border-b border-slate-100 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                        ...
                        <td className="py-3 px-4 text-slate-600 dark:text-slate-400">
                          {patient.responsesCount} answers
                        </td>
                        <td className="py-3 px-4">
              ...
              <div className="text-center py-12">
                <Droplet className="w-12 h-12 mx-auto text-slate-400 mb-4" />
