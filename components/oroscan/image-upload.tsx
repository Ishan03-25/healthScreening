...
export function ImageUpload({ patientInfo, medicalResponses, familyResponses, featureResponses, onSuccess, onBack }: ImageUploadProps) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      // Submit to API
      const response = await fetch("/api/screening/oroscan", {
        method: "POST",
        headers: {
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted dark:from-slate-950 dark:via-slate-900 dark:to-slate-800 transition-colors duration-300">
      ...
      >
        ...
        >
          ...
          >
            <p className="text-foreground dark:text-slate-200 font-medium mb-4">{t("subtitle")}:</p>
